...
func (h *InsurancePolicyHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(`
		SELECT id, user_id, party_id, provider, policy_number, type, coverage_amount,
		       beneficiary, agent_name, agent_phone, location_id, notes, secure_notes, created_at, updated_at
	...
	for rows.Next() {
		...
		var provider, policyNumber, pType, beneficiary, agentName, agentPhone, notes, secureNotes, createdAt, updatedAt string
		var coverageAmount sql.NullFloat64
		if err := rows.Scan(&id, &uid, &partyID, &provider, &policyNumber, &pType, &coverageAmount,
			&beneficiary, &agentName, &agentPhone, &locationID, &notes, &secureNotes, &createdAt, &updatedAt); err != nil {
		...
		item := map[string]any{
			...
			"agent_phone": agentPhone, "notes": notes, "secure_notes": secureNotes,
...
func (h *InsurancePolicyHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	`, policyID, userID).Scan(&id, &uid, &partyID, &provider, &policyNumber, &pType, &coverageAmount,
		&beneficiary, &agentName, &agentPhone, &locationID, &notes, &secureNotes, &createdAt, &updatedAt)
	if err != nil {
...
func (h *InsurancePolicyHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.DB.Exec(`
		INSERT INTO insurance_policies (user_id, party_id, provider, policy_number, type, coverage_amount,
		       beneficiary, agent_name, agent_phone, location_id, notes, secure_notes)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		userID, req.PartyID, req.Provider, req.PolicyNumber, req.Type, req.CoverageAmount,
		req.Beneficiary, req.AgentName, req.AgentPhone, req.LocationID, req.Notes, req.SecureNotes,
	)
	if err != nil {
	...
	json.NewEncoder(w).Encode(map[string]any{
		...
		"location_id": req.LocationID, "notes": req.Notes, "secure_notes": req.SecureNotes,
	})
}
...
func (h *InsurancePolicyHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.DB.Exec(`
		UPDATE insurance_policies SET party_id = ?, provider = ?, policy_number = ?, type = ?,
		       ...
		       location_id = ?, notes = ?, secure_notes = ?
		WHERE id = ? AND user_id = ?
	`, req.PartyID, req.Provider, req.PolicyNumber, req.Type, req.CoverageAmount,
		req.Beneficiary, req.AgentName, req.AgentPhone, req.LocationID, req.Notes, req.SecureNotes, policyID, userID)
	if err != nil {
